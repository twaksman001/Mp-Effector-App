...
effectors_selected = st.multiselect(label='choose effectors',
								    options=effectors_structural,
								    default=effectors_structural,
								    key='chosen effectors'
								    )
...
def get_top_hits(proteins, threshold=0.95, path='Data Files/TT3D/MpEffectors_AtProteome_All_TT3D_positive.txt'):
    
    import pandas as pd
    
    df = pd.read_csv(filepath_or_buffer=path, sep='\t', names=['effector', 'TAIR AGI', 'score'])
    df = df[(df['effector'].isin(proteins)) & (df['score']>threshold)]
...
try:
	df = get_top_hits(proteins=effectors_selected, threshold=threshold)
...
with st.form('select proteins for contact map', clear_on_submit=False):
    effector_select = st.selectbox(label='effector',
                                    options=df['effector'].unique(),
                                    key='effector'
                                    )
...
#     print(protein1, protein2)
    ...
    for file in glob.glob(path2):
        if file_name[:file_name.find('_TT')] in file:
    ...
    if False in [found1, found2]:
        raise ValueError('proteins not found')
